...
from net.Full_IW_MoE_Model import IWMoeNetwork
from net.loss_functions import Full_IW_MOE_Total_Loss
from net.loss import *

from utils.dataSet import FullIWTrainDataset, IWTestDataSet
...
def estimation_flowmap(net, img, epoch):
    net.eval()
    img = cv2.resize(img, (512, 384))
    img = (img / 127.5) - 1.0
    img = torch.from_numpy(img).permute(2, 0, 1).float()
    input_tensor = img.unsqueeze(0).cuda()
    mask = np.ones((512, 384), dtype=np.uint8) * 255
    ...
    task_id_tensor = torch.ones([1], device=input_tensor.device) * 5
    with torch.no_grad():
        flow, final_image, router_logits, resdiue_router_logits, point_cls = net.forward(input_tensor, mask_tensor,criterion.active_residual_tasks, epoch)
    output = flow.detach().cpu().squeeze(0).numpy()
    return output
...
def compute_ori2shape_face_line_metric(model, oriimg_paths, epoch):
    line_all_sum_pred = []
    face_all_sum_pred = []
    for oriimg_path in tqdm(oriimg_paths):
        ...
        # Get the [flow map]"""
        pred = estimation_flowmap(model, input, epoch)
...
def Portrait_val_once(net, epoch, epochs, criterion, optimizer):
    net.eval()
    # iter 1
    oriimg_paths = []
    for root, dirs, files in os.walk(args.fine_tuning_test_path):
    ...
    line_score, face_score = compute_ori2shape_face_line_metric(net, oriimg_paths, epoch)
...
def train_once(net,each_data_batch,epoch,epochs,criterion, optimizer):
    ...
    with tqdm(total=each_data_batch, desc=f'Epoch {epoch + 1}/{epochs}', postfix=dict, mininterval=0.3) as pbar:
        ...
        for i, batch_value in enumerate(train_loader):
            ...
            face_mask = batch_value[5].float().cuda(device=args.device_ids[0])
            face_weight = batch_value[6].float().cuda(device=args.device_ids[0])
            ...
            with torch.amp.autocast('cuda'):
                ...
                total_loss, primary_img_loss, ds_loss, task_loss = criterion(flow, final_image, router_logits,
                                                                                     resdiue_router_logits, task_cls,
                                                                                     ds_flow, gt_tesnor, task_id_tensor,
                                                                                     face_mask, face_weight, epoch)
...
def train(net,saveModelName,criterion,optimizer,scheduler,start_epochs=0, end_epochs=1):
    ...
    for epoch in range(start_epochs,end_epochs):
        ...
        if epoch % 10 == 0:
            ...
            line_score, face_score = Portrait_val_once(net, epoch, end_epochs, criterion, optimizer)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--unified_size', type=list, default=[(256, 256), (256, 256), (256, 256), (512, 384), (512, 384), (512, 384)])
    ...
    parser.add_argument('--train_path', type=str,
                        default=['../../Dataset/WideAngle_Image/training/','../../Dataset/Fisheye_Image/training/',
                                 '../../Dataset/UnrollingShutter/training/',
                                 '../../Dataset/DRC-D/training/', '../../Dataset/DIR-D/training/',
                                 '../../Dataset/Portrait_512/training/'])
    parser.add_argument('--test_path', type=str,
                        default=['../../Dataset/WideAngle_Image/testing/', '../../Dataset/Fisheye_Image/testing/',
                                  '../../Dataset/UnrollingShutter/testing/',
                                 '../../Dataset/DRC-D/testing/', '../../Dataset/DIR-D/testing/'])

    parser.add_argument('--fine_tuning_test_path', type=str,
                        default='../../Dataset/Portrait_512/testing/')
    ...
    for i, test_path in enumerate(args.train_path):
        print(f'Task ID:{i}对应任务:{test_path}')

    # Load Training Data
    train_dataset = FullIWTrainDataset(args.train_path, group_A_tasks, group_B_tasks)
    ...
    # define Loss
    criterion = Full_IW_MOE_Total_Loss(args.lam_appearance, group_A_tasks, group_B_tasks, args.target_perf_ssim, args.target_perf_psnr).cuda(device=args.device_ids[0])
    net = IWMoeNetwork()

        ...
        with tqdm(total=test_each_data_batch, desc=f'Epoch {epoch + 1}/{epochs}', postfix=dict, mininterval=0.3) as t_pbar:
            ...
            with torch.no_grad():
                ...
                for i, batch_value in enumerate(test_loader):
                    ...
                    #
                    #     # 前向传播（修正了typo：input_tesnor -> input_tensor）
                    #     flow, final_image, router_logits, residue_router_logits, task_cls = net.forward(
                    #         input_tesnor,  # 修正拼写错误
                    #         mask_tensor,
                    ...
                    #
                    #     print(f"第 {i + 1} 轮前向传播时间: {forward_time:.4f} 秒")
                    #
                    # # 计算平均值
                    # if forward_times:
                    ...
                    #     print(f"最短时间: {min(forward_times):.4f} 秒")
                    #     print(f"最长时间: {max(forward_times):.4f} 秒")
                    #     print(f"平均时间: {avg_time:.4f} 秒")
                    #     print(f"总时间: {sum(forward_times):.4f} 秒")
...
def train(net,saveModelName,criterion,optimizer,scheduler,start_epochs=0, end_epochs=1):
    ...
    avg_ssim_per_task, avg_psnr_per_task = val_once(net, start_epochs, end_epochs, criterion, optimizer)
